...
- **순서(2026-08 개정)**: 비용 가드레일 → **트랙 A(랜딩존, 저비용)** → 이력서 1차 격상 → 트랙 B(EKS, 단기 집중 후 `destroy`) → SAA는 **마지막**.
- **⚠️ 순서를 바꾼 이유**: JD가 요구하는 건 자격증이 아니라 **경험**입니다. 자격증은 "프로덕션 구축·운영 경험" 칸을 채워주지 못합니다. 랜딩존을 손으로 만들고 나면 SAA 출제 범위의 상당 부분이 이미 몸에 있어 시험 준비가 짧아집니다 — **실물 먼저, 자격증은 마무리**.
...
### 2-5. 2주차 상세 — VPC & 네트워킹 초보자 보강

> 별도 파일 `AWS_네트워킹_초보가이드_VPC서브넷라우팅보안그룹.md`의 압축판. 온프렘 조닝 경험을 AWS 용어로 갈아타는 방식이라 이 영역은 가장 빨리 통과합니다. 막히면 별도 파일에서 그림·상세 콘솔 실습을 참고.

**0) 온프렘 → AWS 한 줄 대응**
IDC 전용망→**VPC**, VLAN/존 분리→**서브넷**, L3 라우팅→**라우트 테이블**, 인터넷 회선→**IGW**, 내부망 아웃바운드 NAT→**NAT 게이트웨이**, 서버 단위 방화벽→**보안 그룹**, 존 단위 방화벽→**NACL**.

**1) VPC/서브넷 핵심 규칙**
- VPC는 **리전** 단위, 서브넷은 **AZ** 단위. CIDR 예: `10.0.0.0/16`.
- **퍼블릭 vs 프라이빗은 스위치가 아니라 "라우팅"으로 결정**: 라우트 테이블에 `0.0.0.0/0 → IGW` 경로가 있으면 퍼블릭, 없으면 프라이빗(나갈 땐 NAT 경유). SAA에서 가장 자주 틀리는 포인트.
- 모든 라우트 테이블엔 자동으로 `VPC CIDR → local`(삭제 불가) — VPC 내부는 항상 서로 통신 가능.

**2) SG vs NACL — 절대 헷갈리면 안 되는 표**

| 구분 | 보안 그룹(SG) | NACL |
|---|---|---|
| 단위 | 인스턴스/ENI | 서브넷 |
| 상태 | **Stateful**(응답 자동 허용) | **Stateless**(인/아웃바운드 각각 규칙 필요) |
| 규칙 | 허용만 | 허용+거부 |
| 특이점 | **다른 SG를 소스로 참조**(예: 앱 SG가 "ALB SG에서 오는 것만" 허용) | IP/포트로만, 번호 순 평가 |

→ SG를 체인으로 걸면(ALB SG → 앱 SG → RDS SG) IP 하드코딩 없이 계층 격리 — 온프렘 조닝의 "존 간 통신 정책"과 동일한 사고. (랩 7 RDS 격리에서 그대로 씀)

**3) 초보가 막히는 3대 지점 (실습 전 미리 인지)**
- "퍼블릭 서브넷인데 인터넷이 안 됨" → 라우트테이블 IGW 경로 / 퍼블릭 IP 할당 / SG 인바운드, 이 3개를 순서대로 점검.
- "프라이빗 인스턴스에서 yum/apt 안 됨" → NAT GW 미생성 또는 프라이빗 RT에 NAT 경로 누락.
- "SG는 열었는데 안 됨" → NACL이 막고 있거나, NACL이 stateless라 리턴 트래픽(ephemeral 포트)이 막힌 경우.

**4) 손으로 하는 미니 랩 (2주차 중 60~90분, NAT GW만 유료)**
- **랩 A**: VPC 1개 + 퍼블릭/프라이빗 서브넷 각 1개 콘솔 수동 구축 → 퍼블릭 EC2는 SSH 성공, 프라이빗 EC2는 직접 접속 실패·NAT로 외부 curl만 성공 확인 → 실습 후 NAT GW/EIP 삭제.
- **랩 B**: SG 체인 실습 — 프라이빗 EC2의 SG 인바운드를 IP가 아닌 "퍼블릭 EC2의 SG"로 설정해 참조 개념 체득.
- **랩 C(선택)**: NACL 아웃바운드 ephemeral 포트를 일부러 막아 "인바운드는 되는데 응답이 안 오는" stateless 현상 재현.
- 콘솔 클릭 경로·검증 명령·CLI 등가는 `AWS_네트워킹_초보가이드_VPC서브넷라우팅보안그룹.md` 5장에 단계별로 있음.

**5) EKS로 넘어갈 때 바로 쓰는 것**
- EKS 노드는 보통 **프라이빗 서브넷**(랩 1의 `privateNetworking: true`). AWS Load Balancer Controller가 서브넷을 자동 인식하려면 태그 필요: 퍼블릭 `kubernetes.io/role/elb=1`, 프라이빗 `kubernetes.io/role/internal-elb=1`. 이 태그 누락이 "Ingress 만들었는데 ALB가 안 생김"의 단골 원인 — 랩 3에서 그대로 만날 함정이니 미리 기억.

---

## 3. 트랙 A — Terraform 랜딩존 (저비용 · 먼저 할 것)

> **왜 EKS보다 먼저인가.** 한화생명·코인원·토스 계열이 요구하는 "클라우드 프로덕션 인프라 구축·운영"의 대부분은 **EKS가 아니라 그 아래층**입니다. VPC·IAM·KMS·환경 분리·감사로그는 EKS 없이 만들 수 있고 상당수가 프리티어 범위라 지출이 거의 없습니다. EKS(트랙 B)는 마지막에 며칠만 띄웠다 지우면 됩니다.
>
> **전제**: 사내 DR 시스템은 **로컬에 그대로 두고 운영**합니다. AWS에는 사내 연동(JIRA/CMDB/Teams/LLM Gateway)을 끊은 더미 버전만 올립니다 — 사내 데이터를 외부 클라우드로 내보내는 것은 보안 규정 위반 소지가 있습니다. 증명하려는 것은 앱 동작이 아니라 **인프라 구성 능력**입니다.

### 랩 A1 — 리포 구조와 환경 분리 (dev/prod)
- **[목표]** 지금 비어 있는 칸 채우기 — 로컬 단일 환경이라 "환경 분리" 경험이 없음 (한화생명 JD 명시 요건)
- **[작업]** 디렉토리로 `envs/dev`, `envs/prod` 분리 + `tfvars`, 원격 상태(S3) + 상태 잠금(DynamoDB), 환경별 state 분리
- **[비용]** 사실상 무료
- **[포인트]** "Terraform 환경별 분리 구성과 원격 상태 잠금" — 팀 협업 전제를 이해한다는 신호

### 랩 A2 — VPC 3-tier + 엔드포인트
- **[목표]** JD "네트워크 분리, VPC·보안그룹·엔드포인트 설계" 직격
- **[작업]** public/private/db 서브넷, 라우팅 테이블, SG 체인(참조 방식). **NAT Gateway 대신 VPC Endpoint(S3/ECR/CloudWatch)** 우선 — NAT은 시간당 과금이라 학습 중엔 띄우지 말 것
- **[포인트]** 온프렘 vSwitch/VLAN·vNIC 존 매핑 경험과 1:1 대응시켜 설명하면 가장 자연스러움

### 랩 A3 — IAM 최소권한 설계
- **[목표]** JD "접근통제(IAM 최소권한)"
- **[작업]** 역할 분리(admin/deploy/readonly), Permissions Boundary, IAM Access Analyzer로 과다권한 탐지
- **[포인트]** ⭐ **전사 IAM 플랫폼 운영 경험과 정면 연결** — 전자결재 기반 발급/회수·최소권한 RBAC을 AWS 정책 언어로 옮긴 것으로 설명

### 랩 A4 — KMS 키 관리와 회전
- **[목표]** JD "KMS 기반 암호화, 키 관리 및 회전 정책 적용"
- **[작업]** 고객관리형 키(CMK) 생성, **자동 회전 활성화**, 키 정책으로 사용 주체 제한, S3/EBS 암호화에 연결
- **[비용]** 키당 월 소액 — 실습 후 삭제 예약(대기기간 있음)
- **[포인트]** WORM·랜섬웨어 방어 체계 경험의 클라우드 대응물

### 랩 A5 — 감사로그·접속기록 보관 체계 ⭐
- **[목표]** JD **필수 경험** "감사로그·접속기록 보관 체계 구성"
- **[작업]** CloudTrail → 전용 S3 버킷, **Object Lock으로 변경 불가**, 수명주기로 Glacier 이관 후 파기
- **[포인트]** ⭐ **가장 강한 카드**. 전 서버 `wtmp`/`btmp` 전수 조사와 WORM 적용 경험이 그대로 대응됩니다. "온프렘에서 하던 증적 보관을 클라우드 네이티브로 옮기면 이렇게 된다"

### 랩 A6 — 데이터 라이프사이클 자동화 (운영/분리보관/파기)
- **[목표]** JD "운영/분리보관/파기 3단 스키마 및 라이프사이클 자동화", 마이데이터 2.0 1년 미접속 삭제
- **[작업]** S3 수명주기 규칙 + Lambda 조건부 파기. **Dry-run을 기본값으로**
- **[포인트]** ⭐ 유휴 자원 회수 파이프라인의 Dry-run 안전장치를 그대로 이식 — "파기 자동화는 오탐이 곧 사고"라는 원칙을 이미 실무에서 세워봤다는 게 차별점

### 랩 A7 — GitHub Actions로 plan / apply
- **[작업]** PR에 `terraform plan` 결과 코멘트, main 머지 시 `apply`. **OIDC로 AWS 인증(액세스 키 없이)**
- **[포인트]** 셀프호스티드 러너 파이프라인 경험의 확장. OIDC 연동은 그 자체로 보안 카드 — 장기 크리덴셜을 없앴다는 이야기

> 💰 **트랙 A 비용**: NAT Gateway만 피하면 월 몇 천 원 수준. 상시 켜두고 천천히 해도 부담이 없어 EKS보다 먼저 하는 게 합리적입니다.

---

## 4. 트랙 B — EKS 실습 (트랙 A 이후)

> 각 랩은 **[목표] → [핵심 작업] → [이력서/면접 포인트]** 구조. 산출물은 GitHub(기존 `sample-app`/`.github` 리포)에 커밋해서 증빙화.

### 4-0. 사전 준비 (랩 0)
- [x] AWS CLI v2, `eksctl`, `kubectl`, `helm`, `terraform` 설치
- [x] IAM 사용자 생성 + `aws configure`(액세스키는 최소권한, 실습 후 폐기)
- [ ] **2-5장(또는 별도 네트워킹 가이드) 랩 A/B 완료 — VPC/서브넷/SG를 손으로 만들어본 뒤 랩 1로 넘어올 것.** 여기서 퍼블릭/프라이빗 판별과 SG 참조를 몸에 익혀두면 랩 1~3의 서브넷 태그·SG 설정에서 헤매지 않음
...
## 5. 실습 → 이력서 격상 매핑 (완료 후 요청하면 일괄 반영)
...
**네트워킹 기초 (2주차)**
- [ ] 온프렘→AWS 대응표 숙지  - [ ] 퍼블릭/프라이빗 판별 기준(라우팅)  - [ ] SG vs NACL 표 암기  - [ ] 랩 A(VPC 수동 구축)  - [ ] 랩 B(SG 체인)  - [ ] 랩 C(NACL stateless, 선택)  - [ ] EKS 서브넷 태그 이해
...
## 8. 지원 전략과의 연결 (2026-08 개정)

### ⚠️ 먼저: 실질 경력은 4년 2개월입니다
CDNetworks는 **인턴**이므로 대부분의 회사가 경력 산정에서 제외합니다. 신세계 4년 2개월이 정규직 경력의 전부이고, 인턴 포함 합산이 5년 2개월입니다.

- 이력서 표기는 `System Engineer, Intern`으로 통일 — 금융권은 고용보험 이력으로 검증하므로 비워두면 신뢰 문제가 됩니다
- **"총 경력 7년" 류의 정량 필터가 있는 자리(한화생명)는 우선순위를 낮출 것**
- **"5년 이상" 명시 자리**도 서류 필터 리스크 — 기술 판단이 앞서는 곳에 힘을 배분

### 지원 목록 티어링
- **A군 (온프렘·트래픽 강점 직격, 연차보다 기술을 봄)**: 넥슨코리아(메이플), SOOP, 가비아, 코인원, CJ ENM
- **B군 (핏은 있으나 클라우드 프로덕션 요구)**: 토스플레이스, 네이버페이, 토스인슈어런스, 힐링페이퍼, 한국신용데이터 → **트랙 A 완료 후 제출이 유리**
- **C군 (HR 정량 필터가 앞섬 / 직무 재확인 필요)**: 한화생명(경력 7년 미달), 아이엠뱅크, 교보문고, 에프앤에프, JYP

> 넥슨 메이플스토리는 **사이렌 오더 이벤트 트래픽 경험이 그대로 통하는** 자리입니다. 목록에서 핏이 가장 좋을 가능성이 높으니 앞으로 당길 것.

### 시점별 행동
- **지금(트랙 A 전)**: A군부터 제출. 온프렘 대규모 운영 + 커널 트러블슈팅 + 보안 거버넌스가 그대로 무기가 되는 곳
- **트랙 A 완료(랩 A1~A7)**: B군 제출. "클라우드 프로덕션 인프라 구축" 칸이 **Terraform 랜딩존 실물**로 채워짐. 특히 랩 A5(감사로그)와 A6(파기 자동화)은 금융권 JD에 직격
- **트랙 B 완료(EKS)**: kind 기반 K8s 근거가 관리형 클라스터 경험으로 확장. 코인원·티빙급 재도전 가능
- **SAA 취득**: 자격증란 반영 + 전 이력서 AWS 표현 일괄 상향. C군 정량 필터 대응용 보조 카드

### 같은 작업 하나가 세 회사를 커버합니다
| 랩 | 한화생명 | 코인원 | 토스 계열 |
|---|:---:|:---:|:---:|
| A1 환경 분리 | ✅ 명시 | | ✅ |
| A2 VPC·SG·엔드포인트 | ✅ 명시 | ✅ | ✅ |
...
| A5 감사로그 보관 | ✅ **필수** | ✅ ISMS | ✅ |
| A6 파기 라이프사이클 | ✅ 마이데이터 | | |
| A7 GitHub Actions OIDC | ✅ 명시 | ✅ 명시 | ✅ |

---

## 9. 현재 시스템의 알려진 제약 (면접 대비)

- **replica 1개** — SQLite는 다중 writer 불가. 늘리려면 PostgreSQL 전환 + 상태 외부화가 선행. 조회 중심 워크로드라 그 복잡도를 감수할 이유가 없다고 판단한 것 → **제약을 알고 내린 결정**으로 답할 것
- **"무중단 배포"로 쓰지 말 것** — RWO 볼륨이라 롤링 시 새 파드가 같은 볼륨을 못 붙음. 이력서 표기는 **"Kubernetes 자동 배포"**로 통일 완료(2026-08)
- **dev/stg/prod 분리 없음** — 로컬 단일 환경. 랩 A1로 해소
- **kind 클러스터** — 프로덕션 트래픽을 받는 클러스터가 아님. "프로덕션 K8s 운영"으로 표기 금지
